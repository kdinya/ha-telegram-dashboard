...
- Section elements can now be reordered on touch/pen devices through the drag handle while preserving desktop drag-and-drop and locked-item behavior; drag labels and order notifications are localized.
- Added a direct touch-event fallback for older mobile WebViews where Pointer Events do not reliably start a reorder gesture.
- Section-item reordering now starts from the full item row on touch devices, while edit/delete/lock controls remain clickable.
- Hardened touch reorder event handling and consolidated duplicate section-item CSS rules.
